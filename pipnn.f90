...
module PES
    implicit none
    ! integer :: 207 = 207 , 15 = 15, 80=80
    double precision, dimension(207, 15) :: weights1
    double precision, dimension(15, 80) :: weights2
    double precision, dimension(80, 1) ::  weights3
    double precision, dimension(1, 15) :: bias1
    double precision, dimension(1, 80) :: bias2
    double precision :: bias3
    double precision, dimension(207+1) :: pdel3a
    double precision, dimension(207+1) :: pavg3a
...
subroutine initialize_potential()
    ...
    do i=1,4
      ...
      read(10,*)(pdel3a(i),i=1,207+1)
      read(10,*)(pavg3a(i),i=1,207+1)
...
subroutine PIP_energy(in, value3, length)
    ...
    double precision, dimension(length, 207) :: in
    double precision, dimension(length, 15) :: value1
    double precision, dimension(length, 80) :: value2
...
subroutine p_scale(p,Nbeads)
    ...
    double precision, intent(inout) :: p(Nbeads,207)
    integer :: i
    do i=1,Nbeads
    p(i,:) =  (p(i,:) - pavg3a(:207)) / pdel3a(:207)
    end do
...
subroutine get_potential(qin,Natoms,Nbeads,V,dVdq,info)
    ...
    double precision :: p0(Nbeads, 207)
    double precision :: p01(Nbeads,207)
    double precision :: p02(Nbeads,207)
    double precision :: p1(3*Natoms*Nbeads,207)
    double precision :: p2(3*Natoms*Nbeads,207)
    integer :: i, j, k
    integer :: start, stop0
    info=0
    q=qin*0.529177210903d0
    ...
    V=V/27.21178d0
    dVdq=dVdq/27.21178d0/ 1.88972612462577d0
    ! write(*,*)p1(:,2)
    ! write(*,*)p2(:,2)
...
subroutine pip(q,p,natoms,Nbeads)
    ...
    double precision :: p_0(Nbeads,0:207)
    double precision, intent(out) :: p(Nbeads,207)
    double precision :: distance(Nbeads,10)
    double precision,dimension(Nbeads,1:10)::x
    integer :: i
...
subroutine distance_vector(q, distance,natoms,Nbeads)
    ...
    integer, intent(in) :: natoms, Nbeads
    double precision,intent(in) :: q(3,natoms,Nbeads)
    double precision,intent(out) :: distance(Nbeads,10)
    integer :: count, i, j, k
    double precision :: d
...
subroutine morse(distance,x,Nbeads)
    implicit none
    integer,intent(in)::Nbeads
    double precision,intent(in)::distance(Nbeads,10)
    double precision,intent(out)::x(Nbeads,10)
    double precision :: alpha=4
    x=dexp(-distance/(alpha*0.5291772d0))
      ...
      subroutine bemsav(x,p)
        ...
        double precision,dimension(1:10),intent(in)::x
        double precision,dimension(0:207),intent(out)::p
        ! ::::::::::::::::::::
        double precision,dimension(0:495)::m
      ...
      subroutine evmono(x,m)
        ...
        integer, parameter :: wp  = 8
        double precision,dimension(1:10),intent(in)::x
        double precision,dimension(0:495),intent(out)::m
        !::::::::::::::::::::
      ...
      subroutine evpoly(m,p)
        implicit none
        integer, parameter :: wp  = 8
        double precision,dimension(0:495),intent(in)::m
        double precision,dimension(0:207),intent(out)::p
        !::::::::::::::::::::
